  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-64 border-r border-gray-100 bg-white md:flex md:flex-col">
      ...
      <nav className="mt-2 flex-1">
        <p className="px-6 pb-2 text-[11px] font-semibold uppercase tracking-wide text-gray-400">Navigation</p>
      ...
      <div className="m-4 rounded-xl border border-gray-100 bg-gray-50 p-3">
        <p className="text-xs font-semibold text-gray-700">Quick Summary</p>
        <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-gray-600">
          <p>Total Task: {summary.tasks}</p>
          <p>Completed: {summary.completed}</p>
          <p>Pending: {summary.pending}</p>
          <p>Deadlines: {summary.deadlines}</p>
          <p className="col-span-2">Due in 7 Days: {summary.dueSoon}</p>
        </div>
        ...
        <div className="mt-3 border-t border-gray-200 pt-3">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Upcoming Deadlines</p>
          <div className="mt-2 space-y-1.5">
            {summary.upcomingDeadlines.length === 0 && <p className="text-[11px] text-gray-500">No upcoming deadlines</p>}
